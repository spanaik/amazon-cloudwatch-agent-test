...
type OtelMetricsClient struct {
	...
	signingService string
	maxRetries     int
	queryDelay     time.Duration // minimum delay between queries to avoid backend pressure
	lastQuery      time.Time
	mu             sync.Mutex
}
...
func (c *OtelMetricsClient) requestRawWithRetry(ctx context.Context, baseURL string, params url.Values) ([]byte, error) {
	// Throttle: ensure minimum delay between queries to avoid backend pressure.
	c.mu.Lock()
	if elapsed := time.Since(c.lastQuery); elapsed < c.queryDelay {
		time.Sleep(c.queryDelay - elapsed)
	}
