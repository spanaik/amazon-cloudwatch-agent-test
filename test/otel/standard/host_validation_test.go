	...
	for _, metricName := range hostEnrichedMetricNames() {
		t.Run(metricName, func(t *testing.T) {
			...
			for _, r := range results {
				...
				requireTrue(t, validTypes[hostType],
	...
	for _, metricName := range hostEnrichedMetricNames() {
		t.Run(metricName, func(t *testing.T) {
			...
			for _, r := range results {
				...
				matchCount++
				requireEqual(t, hostName, nodeName,
					"%s host.name should match k8s.node.name", metricName)
			}
			requireTrue(t, matchCount > 0,
...
func TestUniqueHostIdPerNode(t *testing.T) {
	...
	for _, r := range results {
		...
		if existing, seen := nodeToHostID[node]; seen {
			requireEqual(t, hostID, existing,
	...
	requireTrue(t, len(nodeToHostID) >= 2,
	...
	for _, metricName := range hostEnrichedMetricNames() {
		t.Run(metricName, func(t *testing.T) {
			...
			for _, r := range results {
				...
				requireEqual(t, region, cfg.Region,
	...
	for _, metricName := range hostEnrichedMetricNames() {
		t.Run(metricName, func(t *testing.T) {
			...
			for _, r := range results {
				...
				requireEqual(t, acct, cfg.AccountID,
	...
	for _, metricName := range hostEnrichedMetricNames() {
		t.Run(metricName, func(t *testing.T) {
			...
			for _, r := range results {
				...
				requireTrue(t, az != "", "%s missing cloud.availability_zone", metricName)
				requireTrue(t, strings.HasPrefix(az, cfg.Region),
	...
	for _, metricName := range hostEnrichedMetricNames() {
		t.Run(metricName, func(t *testing.T) {
			...
			for _, r := range results {
				...
				requireEqual(t, provider, "aws", "%s cloud.provider", metricName)
			}
		})
	}
}
	...
	for _, metricName := range hostEnrichedMetricNames() {
		t.Run(metricName, func(t *testing.T) {
			...
			for _, r := range results {
				...
				requireTrue(t, arn != "", "%s missing cloud.resource_id", metricName)
