...
func TestControlPlaneInstrumentation(t *testing.T) {
	for _, metricName := range metricNames(controlPlaneMetrics) {
		t.Run(metricName, func(t *testing.T) {
			...
			requireNonEmpty(t, results, "%s not available (control plane not scraped?)", metricName)
			for _, r := range results {
				name, ok := r.Labels.Instrumentation["@name"]
				requireTrue(t, ok, "%s missing @instrumentation.@name", metricName)
				requireEqual(t, name, scopePrometheus, "%s instrumentation name", metricName)
			}
		})
	}
}
...
func TestAPIServerComponentLabel(t *testing.T) {
	for _, metricName := range metricNames(controlPlaneMetrics) {
		t.Run(metricName, func(t *testing.T) {
			results, err := queryCache.Get(context.Background(), metricName)
			requireNoError(t, err, "querying %s", metricName)
			requireNonEmpty(t, results, "%s not available (apiserver not scraped?)", metricName)
			for _, r := range results {
				comp, ok := r.Labels.Resource["k8s.component.name"]
				requireTrue(t, ok, "%s missing @resource.k8s.component.name", metricName)
				requireEqual(t, comp, "apiserver", "%s k8s.component.name", metricName)
			}
		})
	}
}
	...
	for _, md := range controlPlaneMetrics {
		...
		for _, label := range md.ExpectedLabels {
			t.Run(md.Name+"/"+label, func(t *testing.T) {
				...
				requireNonEmpty(t, results, "%s not available (control plane not scraped?)", md.Name)
				for _, r := range results {
