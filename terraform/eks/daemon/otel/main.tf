...
resource "aws_iam_role_policy_attachment" "pod_identity_AWSXrayWriteOnlyAccess" {
...
resource "null_resource" "validator" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      echo "=== Debug: cluster-scraper logs (last 10) ==="
      kubectl logs -n amazon-cloudwatch -l app.kubernetes.io/name=cloudwatch-agent-cluster-scraper --tail=10 || true
      echo "========================="
      ...
      echo "=== Sleeping 30 minutes for debugging ==="
      echo "=== Cluster: ${aws_eks_cluster.this.name} Region: ${var.region} ==="
      sleep 1800
